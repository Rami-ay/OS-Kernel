...
//--------------------------------------------------------
void do_child(int i, int array[],int pipe_descs[])
{
	//We close the pipe_desc[0] so the children
	//cant write on the reading elemnt in pipe_descs:
	close(pipe_descs[0]);

	//closing stdout and the output is the pipe_descs[1]:
	close(STDOUT_FILENO);
	dup(pipe_descs[1]);
	...
	//Done writing on pipe:
	close(pipe_descs[1]);
	exit(EXIT_SUCCESS);
}
//--------------------------------------------------------
void father(int number)
{
	struct timeval t0, t1;
	pid_t status;
	char c;
	...
	for (int time = 0; time < 10; time++)
	{
		...
		//====================================
		//now children writes on father stdin:
		//====================================
	...
	dup(pipe_descs[0]);
	//---------------------------------------
	//now we have a file of 20 rows in stdin:
	//---------------------------------------
	...
	while (c != EOF)
	{
		...
		if (c == 's')
		{
			...
			scanf("%d", &throw); //throw the found numbers
			getchar(); // throw the second the second ' '
			scanf("%f", &temp_number); //store the time
	...
	// printing the mean of s and b searches:
